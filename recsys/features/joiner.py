...
class FeatureJoiner:
    ...
    def transform(self, pairs_train: pd.DataFrame, pairs_test: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        for frame in (pairs_train, pairs_test):
            if "ts_query" in frame.columns:
                frame["ts_query"] = pd.to_datetime(frame["ts_query"])
        frames_train = []
        frames_test = []
        ...
        if merged_train.isna().values.any() or merged_test.isna().values.any():
            raise ValueError("NaN detected after feature generation; expected deterministic imputations")
